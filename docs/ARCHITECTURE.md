...
The side panel observes active-tab and navigation state through extension APIs, derives a canonical page identity, loads cached local data immediately, and delegates edits to application services. One app-lifetime ownership coordinator sits outside the React tree, registers the current draft with the navigation flush boundary, interrupts stale startup, and preserves failed cleanup across StrictMode or same-realm remounts until an explicit retry succeeds. On exact-origin roots, one app-lifetime recent-note index queries the shared note service, excludes the root and tombstones, coalesces relevant `chrome.storage.local` change bursts, and retains immutable prior entries through retryable refresh failures; canonical entries open through a credential-free HTTP(S)-only tabs adapter. A passive sync-visibility controller observes only the active page's exact queue key plus connection metadata, coalesces change bursts, and treats the queue entry as authoritative even when BYOS is disconnected or unavailable. The service worker configures toolbar behavior, resumes a pending identity migration once per worker boot, owns the production BYOS/S3/queue/engine composition, and reconciles on boot, install, startup, connection, completed identity migration, panel opening, successful local mutation, periodic alarm, and one-shot retry alarm. The options page loads versioned settings, patches editor mode atomically against the latest record, executes custom identity rules through the migration service, controls BYOS connection state, invalidates worker credentials before local disconnect, and reports the live aggregate queue count without invoking synchronization directly.
...
`SyncEngine` compares local and remote records by `savedAt`, then lexicographic `revisionId` when timestamps are equal. Same-version divergent records become explicit integrity conflicts. It validates and reconciles the sorted local/remote/queue union, writes local winners remotely, applies remote winners through a complete-record atomic local CAS, removes queue-only orphans with revision CAS, continues independent pages after partial failures, and keeps temporary S3 secrets only in service-worker memory. The versioned queue stores only page key, current revision ID, bounded attempt count, and next-attempt timestamp; the entry is the sole per-page unsynced flag, repeated intent coalesces, and retry timestamps use injected exponential backoff rather than active polling. Passive UI status reloads validated repository state instead of trusting storage event payloads, never adds another durable sync flag, and advances a post-save remote claim only after observing that save's pending revision disappear or a strictly newer successful-sync watermark. The runtime binds each engine run to one immutable OAuth connection identity, verifies it before and after credential issuance, invalidates credentials and stale metadata completion across disconnect/reconnect generations, and advances only due exact queue revisions when a whole reconciliation attempt fails.
