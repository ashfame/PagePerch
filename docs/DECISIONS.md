...
## 2026-07-29 — Load Gutenberg Only After a Supported Page Reaches Editor Composition

- Decision: Keep the application shell and page-identity session eager, but pass a small lazy editor adapter into the shell so `PageNoteEditor`, its Gutenberg graph, and its editor-only stylesheet are imported only after the supported-page render path mounts that adapter.
- Context: Registry reduction made the emitted editor graph smaller but did not materially change mounted heap, while every unsupported panel still evaluated Gutenberg despite never rendering an editor.
- Options considered: Eagerly import the editor for simpler state timing; dynamically import it inside the editor component after its module has already evaluated; lazily import the entire application shell; inject a cached asynchronous editor adapter at the existing UI/application boundary.
- Consequences: Unsupported and pre-editor states retain immediate lightweight shell behavior without fetching or evaluating editor resources; a supported page keeps the existing loading ownership, errors, focus, autosave, navigation, and persistence contracts. Production and browser audits fail if the editor becomes eager again. A page reload is the recovery path for the exceptional editor-chunk import failure.
- Follow-up tasks: Reduce the lazy editor stylesheet in PP-029. Do not expand this boundary into a replacement editor shell or any BYOS, synchronization, authentication, AWS/S3, or service-worker change.

## 2026-07-29 — Replace Aggregate Gutenberg Block CSS with an Exact Supported Surface

- Decision: Retain upstream Gutenberg component, block-editor, format-library, and edit-post structural CSS, but replace the aggregate block-library style/editor/theme bundles with exact common editor elements and every available style family for PagePerch's eight registered text blocks.
- Context: The isolated editor's complete stylesheet shipped styles for every core block even though PagePerch registers only paragraph, heading, list, list item, quote, code, preformatted, and separator.
- Options considered: Keep the complete stylesheet; hand-copy only selectors seen in today's DOM; purge unused selectors heuristically; preserve upstream structural layers and explicitly import the supported block families.
- Consequences: Editor CSS is 41.6% smaller while upstream behavior remains maintainable and every packaged editor scenario passes. Build audits bind the exact ordered import surface, representative required and unsupported output markers, and maximum byte budgets; they intentionally do not bind declaration values or visual appearance. Manual visual acceptance remains required for the packaged candidate.
- Follow-up tasks: Keep the import manifest aligned with the exact JavaScript block registry when supported blocks change. Do not use this decision to attempt the deferred direct editor shell or change BYOS, synchronization, authentication, AWS/S3, or service-worker behavior.
