...
The production executor advances `planned`, `destinations-applied`, `tombstones-applied`, and `settings-applied` phases behind the shared Chrome storage lock, then verifies the final records/index/settings before removing the journal. The service worker resumes a pending plan once per worker boot. Note writes, note deletes, and direct settings writes check for any journal value inside that same lock and reject until recovery finishes; reads remain available.
