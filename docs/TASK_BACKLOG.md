...
- Notes: PP-004A through PP-004C1 are accepted: a UI-neutral active-page session controller owns current-window routing and flush ordering; narrow Chrome/settings adapters and a StrictMode-safe accessible React shell compose loading, unsupported, error, and keyed supported states; and the standalone real Gutenberg adapter enforces exact block modes, recursively normalizes stored content to safe local text blocks, captures first edits, blocks unhandled WordPress API network access, remounts on mode changes, and supplies accessible light/dark/narrow/reduced-motion styling. Pending-flush errors retain the prior keyed document, settings-launch errors are visible and retryable, and the direct extension-page smoke covers keyboard focus without assuming tab order. The adapter remains intentionally unmounted until PP-004C2 atomically adds cached note loading, pending editor state, 750 ms local autosave/status UI, and real flush integration; the root index follows afterward.
