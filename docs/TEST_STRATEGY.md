...
- Repository and service tests run against a faithful in-memory mock of `chrome.storage.local` and `chrome.storage.session`, including change events, error paths, large Gutenberg documents, restart reconstruction, migration recovery from every journal phase, and shared-lock mutation exclusion.
...
- Transport tests mock `fetch`, Chrome identity, and AWS SDK request handling while asserting exact OAuth form fields/scope/callback state, suspension-safe PKCE, token and credential lifetime, disconnect cancellation, path-style S3 configuration, deterministic keys, and partial failure behavior without making production requests.
