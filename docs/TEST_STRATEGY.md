...
- React component tests use React Testing Library with explicit fake timers and `matchMedia`, Chrome API, repository, and service dependencies to verify user-visible state, keyboard behavior, focus, automatic theme changes, navigation, root-index loading/refresh/open failures, options validation/migrations, stale-recovery-safe editor settings, and BYOS configuration, Connect/Reconnect/Disconnect, exact-expiry, status-refresh recovery, and config-less cleanup controls.
- Transport and synchronization tests mock `fetch`, Chrome identity, AWS SDK request handling, local/remote repositories, clocks, and retry randomness while asserting exact OAuth form fields/scope/callback state, suspension-safe PKCE, token and credential lifetime, disconnect cancellation, path-style S3 configuration, deterministic keys/winners, tombstones, bounded retry, restart recovery, concurrent-save preservation, partial failure continuation, integrity conflicts, and redacted errors without making production requests.
